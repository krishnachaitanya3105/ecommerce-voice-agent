...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at  EasyBuy Mart:"]
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at EasyBuy Mart:"]
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Yogi', the friendly shopkeeper and voice assistant for EasyBuy Mart.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT (EasyBuy Mart) — Yogi")
